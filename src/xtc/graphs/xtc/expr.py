...
class XTCTensorExpr(XTCValueExpr):
    def __init__(
        ...
        if tensor is None:
            ...
            type = XTCTensorType(shape=shape, dtype=dtype, device=device, const=const)
            value = XTCTensor(type=type)
        elif isinstance(tensor, XTCTensorType):
            assert shape is None and dtype is None
            assert not const, "const cannot be set when tensor type is provided directly"
            value = XTCTensor(type=tensor)
        elif isinstance(tensor, XTCTensor):
            assert shape is None and dtype is None
            assert not const, "const cannot be set when tensor value is provided directly"
            value = tensor
            ...
            if shape is not None:
                ...
                type = XTCTensorType(
                    shape=tensor, dtype=shape, device=device, const=const
                )
            else:
                type = XTCTensorType(
                    shape=tensor, dtype=dtype, device=device, const=const
                )
            value = XTCTensor(type=type)
        self._value = value
        self._device = device
